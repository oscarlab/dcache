...
--------------------------------------------------------------------
*/
u32 jelkins_lookup3_hashword(
	const u32 *k,	/* the key, an array of u32 values */
	int length,	/* the length of the key, in u32s */
...
/*
--------------------------------------------------------------------
hashword64() -- same as hashword(), but take two seeds and return two
32-bit values.  pc and pb must both be nonnull, and *pc and *pb must
both be initialized with seeds.  If you pass in (*pb)==0, the output 
(*pc) will be the same as the return value from hashword().
--------------------------------------------------------------------
*/
void jelkins_lookup3_hashword64 (
	const u32 *k,	/* the key, an array of u32 values */
	int length,	/* the length of the key, in u32s */
...
u32 jelkins_lookup3_hash(const void *key, int length, u32 initval)
{
	...
	if (HASH_LITTLE_ENDIAN && ((u.i & 0x3) == 0)) {
		...
		const u16 *k = (const uint16_t *)key;         /* read 16-bit chunks */
		const u8 *k8;
...
/*
 * hash64: return 2 32-bit hash values
 *
...
{
	u32 a, b, c;
	union { const void *ptr; int i; } u; /* to cast key to (size_t) happily */
